...
async function backupFolder(pool, drive) {
  ...
  if (!id) {
    ...
    id = created.data?.id || "";
    if (!id || !(created.data.parents || []).includes(PARENT_FOLDER_ID)) throw new Error("Namenske Drive mape za backup ni mogoce varno ustvariti.");
  }
